...
import { BookOpen, ArrowLeft, BookMarked, Edit, Trash2, Eye, AlertTriangle, X, Plus, Link2, Copy } from 'lucide-react'
...
export default function NoteDetail() {
  ...
  const fetchNote = async () => {
    try {
      const response = await fetch(`/api/classes/${params.classId}/notes/${params.noteId}`)
      if (response.ok) {
        ...
        setFormData({
          ...
          notesLinks: noteData.notesLinks
        })
      } else {
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 relative overflow-hidden">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-slate-300 mt-1 font-medium">
                  {isEditing ? 'Update your note information' : 'View and manage your note'}
                </p>
              </div>
            </div>
            ...
            {isEditing ? (
              <form className="space-y-6">
                ...
                <div>
                  ...
                  <input
                    ...
                    className={`w-full px-4 py-3 bg-slate-700/50 backdrop-blur-lg border rounded-xl focus:ring-2 focus:ring-blue-500/50 focus:border-blue-400/50 text-slate-100 placeholder-slate-400 font-medium shadow-lg transition-all duration-200 ${errors.noteName ? 'border-red-500/70 focus:ring-red-500/50' : 'border-slate-600/50'
                      }`}
                    required
                  />
                  {errors.noteName && (
                ...
                <div>
                  ...
                  <input
                    ...
                    className={`w-full px-4 py-3 bg-slate-700/50 backdrop-blur-lg border rounded-xl focus:ring-2 focus:ring-blue-500/50 focus:border-blue-400/50 text-slate-100 placeholder-slate-400 font-medium shadow-lg transition-all duration-200 ${errors.noteTopic ? 'border-red-500/70 focus:ring-red-500/50' : 'border-slate-600/50'
                      }`}
                    required
                  />
                  {errors.noteTopic && (
                      ...
                      <div key={index} className="flex gap-2">
                        ...
                        {formData.notesLinks.length > 1 && (
                          <button
                            ...
                            className="p-3 bg-red-600/20 hover:bg-red-600/30 border border-red-500/30 rounded-xl text-red-400 transition-all duration-200 shadow-lg"
                          >
                            <X className="w-4 h-4" />
                          </button>
                        )}
              ...
              <div className="space-y-6">
                ...
                <div className="bg-slate-700/30 backdrop-blur-lg rounded-xl p-6 border border-slate-600/50">
                  ...
                  <div className="space-y-3">
                    {note.notesLinks.map((link, index) => (
                      <div key={index} className="flex items-center gap-3 p-3 bg-slate-600/30 rounded-lg border border-slate-600/30">
                        <a
                          href={link}
                          target="_blank"
                          rel="noopener noreferrer"
                          className="flex-1 text-blue-400 hover:text-blue-300 underline break-all font-medium transition-colors duration-200"
                        >
                          {link}
                        </a>
                        <button
                ...
                <div className="bg-slate-700/30 backdrop-blur-lg rounded-xl p-6 border border-slate-600/50">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
                    <div>
                      ...
                      <p className="text-slate-200 font-semibold">{note.creator.name}</p>
                    </div>
                    <div>
                      <p className="text-slate-400 font-medium">Email</p>
                      <p className="text-slate-200 font-semibold">{note.creator.email}</p>
                    </div>
                    <div>
                      <p className="text-slate-400 font-medium">Created</p>
                      <p className="text-slate-200 font-semibold">{new Date(note.createdAt).toLocaleString()}</p>
                    </div>
                    {note.updatedAt !== note.createdAt && (
                      <div>
                        <p className="text-slate-400 font-medium">Last Updated</p>
                        <p className="text-slate-200 font-semibold">{new Date(note.updatedAt).toLocaleString()}</p>
                      </div>
                    )}
                  </div>
                </div>
        ...
        {isEditing && (
          <div className="mt-6 bg-blue-500/10 backdrop-blur-lg border border-blue-400/30 rounded-xl p-4">
            ...
            <ul className="text-blue-100 text-sm space-y-1 font-medium">
              <li>• Choose a clear and descriptive note name</li>
              <li>• Specify the topic or subject area clearly</li>
              <li>• Add relevant links to external resources</li>
              <li>• Ensure all links are valid and accessible</li>
              <li>• Remove empty link fields before saving</li>
            </ul>
          </div>
        )}
      </main>
            ...
            <div className="p-6">
              ...
              <div className="flex items-center justify-between mb-4">
                <div className="flex items-center gap-3">
                  ...
                  <h3 className="text-xl font-bold text-slate-100">Delete Note</h3>
                </div>
                <button
      ...
      {loading && (
        <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-sm z-40 flex items-center justify-center">
          <LoadingSpinner message={isEditing ? "Updating note..." : "Processing..."} />
        </div>
      )}
