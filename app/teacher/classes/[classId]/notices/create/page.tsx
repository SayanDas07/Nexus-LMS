...
export default function CreateNotice() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch(`/api/classes/${params.classId}/notices`, {
        method: 'POST',
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 relative overflow-hidden">
      ...
      <div className="bg-slate-800/60 backdrop-blur-lg border-b border-slate-700/50 relative z-10 shadow-xl">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 sm:py-6">
          <div className="flex items-center gap-3">
            <Bell className="w-8 h-8 text-blue-400 drop-shadow-lg" />
            <div>
              <h1 className="text-2xl sm:text-3xl font-bold bg-gradient-to-r from-slate-100 to-blue-200 bg-clip-text text-transparent">
                Create Notice
              </h1>
              <p className="text-slate-300 mt-1 font-medium">Share important information with your students</p>
            </div>
          </div>
        </div>
      </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div>
                ...
                <input
                  ...
                  className={`w-full px-4 py-3 bg-slate-700/50 backdrop-blur-lg border rounded-xl focus:ring-2 focus:ring-blue-500/50 focus:border-blue-400/50 text-slate-100 placeholder-slate-400 font-medium shadow-lg transition-all duration-200 ${errors.title ? 'border-red-500/70 focus:ring-red-500/50' : 'border-slate-600/50'
                    }`}
                  required
                />
                {errors.title && (
              ...
              <div>
                ...
                <textarea
                  ...
                  rows={8}
                  className={`w-full px-4 py-3 bg-slate-700/50 backdrop-blur-lg border rounded-xl focus:ring-2 focus:ring-blue-500/50 focus:border-blue-400/50 text-slate-100 placeholder-slate-400 font-medium shadow-lg transition-all duration-200 resize-none ${errors.content ? 'border-red-500/70 focus:ring-red-500/50' : 'border-slate-600/50'
                    }`}
                  required
                />
                ...
                <div className="flex justify-between items-center mt-2">
                  ...
                  <div className="text-right">
                    <p className={`text-sm font-medium ${isWordCountExceeded
                        ? 'text-red-400'
                        : wordCount > 80
                          ? 'text-amber-400'
                          : 'text-slate-400'
      ...
      {loading && (
        <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-sm z-40 flex items-center justify-center">
          <LoadingSpinner message="Creating notice..." />
        </div>
      )}
